...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        Map<String, Post> posts = initPosts(categories, users);
        Map<String, Comment> comments = initComments(posts, users);
        initLikesAndViews(posts, users, comments);
        initBookmarks(posts, users);
        initNotifications(posts, users, comments);
        initReports(posts, users, comments);
    }

    private Map<String, Category> initCategories() {
        Map<String, Category> map = new LinkedHashMap<>();
        map.put("notice", categoryRepository.save(new Category("공지")));
        map.put("free", categoryRepository.save(new Category("자유")));
        map.put("question", categoryRepository.save(new Category("질문")));
        map.put("info", categoryRepository.save(new Category("정보공유")));
        return map;
    }
    ...
    private Map<String, User> initUsers() {
        ...
        map.put("admin", userRepository.save(User.builder()
        ...
        map.put("user1", userRepository.save(User.builder()
                .email("test1@test.com")
                .password(passwordEncoder.encode("1234"))
                .nickname("개발초보")
                .build()));

        map.put("user2", userRepository.save(User.builder()
                .email("test2@test.com")
                .password(passwordEncoder.encode("1234"))
                .nickname("코딩마스터")
                .build()));

        map.put("user3", userRepository.save(User.builder()
                .email("test3@test.com")
                .password(passwordEncoder.encode("1234"))
                .nickname("자바킹")
                .build()));

        map.put("user4", userRepository.save(User.builder()
                .email("test4@test.com")
                .password(passwordEncoder.encode("1234"))
                .nickname("리액트러버")
                .build()));

        map.put("user5", userRepository.save(User.builder()
    ...
    private Map<String, Post> initPosts(Map<String, Category> categories, Map<String, User> users) {
        Map<String, Post> map = new LinkedHashMap<>();
        Category notice = categories.get("notice");
        Category free = categories.get("free");
        Category question = categories.get("question");
        Category info = categories.get("info");
        ...
        User user5 = users.get("user5");

        // 공지글
        map.put("noticePost1", postRepository.save(Post.builder()
                .user(admin).category(notice)
                .title("[필독] 게시판 이용규칙 안내")
                .content("안녕하세요, 관리자입니다.\n\n1. 욕설, 비방 금지\n2. 광고성 게시글 금지\n3. 중복 게시글 금지\n4. 개인정보 노출 금지\n\n위반 시 경고 없이 게시글이 삭제될 수 있습니다.\n감사합니다.")
                .build()));

        map.put("noticePost2", postRepository.save(Post.builder()
                .user(admin).category(notice)
        ...
        map.put("p6", postRepository.save(Post.builder()
                .user(user1).category(free)
                .title("오늘 드디어 첫 프로젝트 완성!!")
                .content("투두리스트 앱인데 완성했습니다!!\nCRUD 전부 구현했고 CSS도 나름 꾸몄어요\n별거 아닌 거지만 뿌듯하네요 ㅎㅎ")
                .build()));

        map.put("p7", postRepository.save(Post.builder()
                .user(user2).category(free)
        ...
        // 질문 게시판
        map.put("p8", postRepository.save(Post.builder()
        ...
        map.put("p9", postRepository.save(Post.builder()
        ...
        map.put("p10", postRepository.save(Post.builder()
        ...
        map.put("p11", postRepository.save(Post.builder()
        ...
        map.put("p12", postRepository.save(Post.builder()
                .user(user1).category(question)
    ...
    private Map<String, Comment> initComments(Map<String, Post> posts, Map<String, User> users) {
        ...
        User user4 = users.get("user4");
        User user5 = users.get("user5");
        Post p1 = posts.get("p1");
        Post p2 = posts.get("p2");
        Post p3 = posts.get("p3");
        Post p5 = posts.get("p5");
        Post p6 = posts.get("p6");
        Post p8 = posts.get("p8");
        Post p9 = posts.get("p9");
        Post p13 = posts.get("p13");
        Post p16 = posts.get("p16");
        Post p18 = posts.get("p18");
        Post p19 = posts.get("p19");
        Post p20 = posts.get("p20");
    ...
    private void initLikesAndViews(Map<String, Post> posts, Map<String, User> users, Map<String, Comment> comments) {
        User user1 = users.get("user1");
        User user2 = users.get("user2");
        User user3 = users.get("user3");
        User user4 = users.get("user4");
        User user5 = users.get("user5");
        Post p2 = posts.get("p2"); Post p3 = posts.get("p3"); Post p4 = posts.get("p4");
        Post p5 = posts.get("p5"); Post p6 = posts.get("p6"); Post p7 = posts.get("p7");
        Post p8 = posts.get("p8"); Post p9 = posts.get("p9"); Post p10 = posts.get("p10");
        Post p11 = posts.get("p11"); Post p12 = posts.get("p12"); Post p13 = posts.get("p13");
        Post p14 = posts.get("p14"); Post p15 = posts.get("p15"); Post p16 = posts.get("p16");
        Post p17 = posts.get("p17"); Post p18 = posts.get("p18"); Post p19 = posts.get("p19");
        Post p20 = posts.get("p20");

        // 조회수 (ViewLog + viewCount 직접 반영)
        addView(user1, p13); addView(user2, p13); addView(user3, p13); addView(user4, p13); addView(user5, p13);
        addView(user1, p3); addView(user2, p3); addView(user4, p3); addView(user5, p3);
        addView(user1, p5); addView(user2, p5); addView(user3, p5); addView(user4, p5);
        addView(user1, p14); addView(user2, p14); addView(user4, p14); addView(user5, p14);
        addView(user1, p16); addView(user2, p16); addView(user3, p16); addView(user4, p16);
        addView(user1, p18); addView(user2, p18); addView(user4, p18);
        addView(user1, p8); addView(user3, p8); addView(user4, p8);
        addView(user2, p6); addView(user3, p6); addView(user5, p6);
        addView(user1, p2); addView(user3, p2); addView(user5, p2);
        addView(user1, p20); addView(user2, p20); addView(user3, p20);
        addView(user1, p9); addView(user2, p9);
        addView(user2, p17); addView(user5, p17);
        addView(user1, p15); addView(user3, p15);
        addView(user2, p4); addView(user5, p4);
        addView(user3, p7); addView(user4, p7);
        addView(user1, p19);
        addView(user3, p11); addView(user4, p11);
        addView(user2, p12); addView(user4, p12);
        addView(user1, p10); addView(user3, p10);
        ...
        // 댓글 좋아요
        Comment c5 = comments.get("c5");
        Comment c9_1 = comments.get("c9_1");
        Comment c4 = comments.get("c4");
        Comment c1 = comments.get("c1");
        Comment c13_1 = comments.get("c13_1");
        Comment c6_1 = comments.get("c6_1");
        Comment c20_1 = comments.get("c20_1");
        Comment c5r1 = comments.get("c5r1");
        Comment c16_1 = comments.get("c16_1");

        addCommentLike(user1, c5);
        addCommentLike(user4, c5);
        addCommentLike(user1, c9_1);
        addCommentLike(user4, c9_1);
        addCommentLike(user1, c4);
        addCommentLike(user5, c4);
        addCommentLike(user2, c1);
        addCommentLike(user1, c13_1);
        addCommentLike(user3, c6_1);
        addCommentLike(user5, c6_1);
        addCommentLike(user3, c20_1);
        addCommentLike(user5, c20_1);
        addCommentLike(user2, c5r1);
        addCommentLike(user1, c16_1);
    }

    private void initBookmarks(Map<String, Post> posts, Map<String, User> users) {
        User user1 = users.get("user1");
        User user2 = users.get("user2");
        User user3 = users.get("user3");
        User user4 = users.get("user4");
        ...
        // user1(개발초보) - 공부자료 북마크
        bookmarkRepository.save(new Bookmark(user1, posts.get("p13")));
        bookmarkRepository.save(new Bookmark(user1, posts.get("p14")));
        bookmarkRepository.save(new Bookmark(user1, posts.get("p16")));
        bookmarkRepository.save(new Bookmark(user1, posts.get("p15")));
        bookmarkRepository.save(new Bookmark(user1, posts.get("p17")));

        // user2(코딩마스터) - 정보글 위주
        bookmarkRepository.save(new Bookmark(user2, posts.get("p14")));
        bookmarkRepository.save(new Bookmark(user2, posts.get("p16")));

        // user3(자바킹) - 기술 관련
        bookmarkRepository.save(new Bookmark(user3, posts.get("p13")));
        bookmarkRepository.save(new Bookmark(user3, posts.get("p8")));
        bookmarkRepository.save(new Bookmark(user3, posts.get("p17")));

        // user4(리액트러버) - 프론트 관련
        bookmarkRepository.save(new Bookmark(user4, posts.get("p15")));
        bookmarkRepository.save(new Bookmark(user4, posts.get("p13")));

        // user5(취준생김철수) - 취업 관련
        bookmarkRepository.save(new Bookmark(user5, posts.get("p13")));
        bookmarkRepository.save(new Bookmark(user5, posts.get("p16")));
        bookmarkRepository.save(new Bookmark(user5, posts.get("p14")));
        bookmarkRepository.save(new Bookmark(user5, posts.get("p5")));
    }

    private void initNotifications(Map<String, Post> posts, Map<String, User> users, Map<String, Comment> comments) {
        User user1 = users.get("user1");
        User user2 = users.get("user2");
        User user3 = users.get("user3");
        User user4 = users.get("user4");
    ...
    private void initReports(Map<String, Post> posts, Map<String, User> users, Map<String, Comment> comments) {
        User user3 = users.get("user3");
        User user5 = users.get("user5");
        Comment c1 = comments.get("c1");
        Comment c6 = comments.get("c6");
