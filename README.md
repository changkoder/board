...
**Spring Boot 기반 커뮤니티 게시판**

JWT 인증 · Querydsl 동적 쿼리 · 관리자 시스템 · 알림 · 신고 자동숨김

[![Java](https://img.shields.io/badge/Java-17-ED8B00?style=flat&logo=openjdk&logoColor=white)](https://openjdk.org/)
[![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.3.7-6DB33F?style=flat&logo=springboot&logoColor=white)](https://spring.io/projects/spring-boot)
...
[![JPA](https://img.shields.io/badge/JPA-Querydsl%205.1-59666C?style=flat&logo=hibernate&logoColor=white)](http://querydsl.com/)
...
| 기능 | 설명 |
|---|---|
| **게시글** | CRUD, 검색(제목/내용/작성자), 인기글, 카테고리 필터, 무한스크롤 |
| **댓글** | 대댓글 (2-depth 계층 구조), @멘션 알림 |
| **좋아요 / 북마크** | 토글 방식, 비정규화 카운트, 알림 연동 |
| **이미지** | 다중 업로드, 에디터 미리보기, 게시글과 분리된 API |
| **알림** | 댓글 · 좋아요 · 답글 · 멘션, 중복 방지, 스냅샷 저장 |
| **신고** | 5회 누적 시 자동 숨김, 이중 방어 (서비스 + DB 제약) |
| **관리자** | 숨김/복원/강제삭제, 유저 차단, 신고 내역 상세조회 |
...
```
  ...
  └──────┬───────┘           └──────┬───────┘          └──────────────┘
         ...
         └────────────┐  ┌─────────┘
                 ...
                 ┌──────────────┐
                 │   Comment    │◀──┐
                 └──────────────┘   │ parent_id (자기참조, 2-depth)
                        │ ──────────┘
                        │

  ── User·Post·Comment 사이의 행위 기록 (모두 User FK + 대상 FK) ──

  ┌──────────┐  ┌─────────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
  │ PostLike │  │ CommentLike │  │ Bookmark │  │ ViewLog  │  │  Report  │
  └──────────┘  └─────────────┘  └──────────┘  └──────────┘  └──────────┘
   User ↔ Post   User ↔ Comment   User ↔ Post  User ↔ Post  User ↔ Post
                                                             User ↔ Comment
  ...
  └────────────────┘  └───────────────┘
   FK: User(수신자)     userId + token
   Long: actorId,
     postId, commentId
```

> 엔티티 필드 상세는 [ERD 문서](docs/ERD.md) 참고
...
>
> 전체 API 목록은 [API 문서](docs/API.md) 참고
...
> **총 15개 파일 · 119개 테스트 메서드** — `@SpringBootTest` + H2 통합 테스트
