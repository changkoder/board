...
> 엔티티 12개 · 모든 `@ManyToOne`은 `LAZY` · Soft delete 적용: User, Post, Comment
...
<details>
<summary><b>엔티티 필드 상세 (클릭하여 펼치기)</b></summary>

### 핵심 엔티티

| 엔티티 | 주요 필드 | 연관관계 | 비고 |
|---|---|---|---|
| **User** | email, password, nickname, profileImg, role(`USER`/`ADMIN`), status(`ACTIVE`/`BLOCKED`), postCount, deleted | — | BaseEntity 상속 |
| **Post** | title, content, viewCount, likeCount, commentCount, hidden, deleted | `@ManyToOne` → User, Category | BaseEntity 상속. 비정규화 카운트 3개 |
| **PostImage** | imageUrl, imageOrder | `@ManyToOne` → Post | `cascade=ALL`, `orphanRemoval=true` |
| **Comment** | content, likeCount, hidden, deleted | `@ManyToOne` → User, Post, Comment(parent) | 자기참조 2-depth 대댓글 |
| **Category** | name | — | BaseEntity 미상속 |

### 행위 기록 엔티티

> 모두 `@UniqueConstraint`로 유저당 1회 제한. BaseEntity 미상속 (수정 없는 도메인).

| 엔티티 | FK | 유니크 제약 |
|---|---|---|
| **PostLike** | user_id, post_id | `UK(user_id, post_id)` |
| **CommentLike** | user_id, comment_id | `UK(user_id, comment_id)` |
| **Bookmark** | user_id, post_id | `UK(user_id, post_id)` |
| **ViewLog** | user_id, post_id | `UK(user_id, post_id)` |

### 이벤트 / 인증 엔티티

| 엔티티 | 주요 필드 | 설계 특이점 |
|---|---|---|
| **Notification** | type(`COMMENT`/`REPLY`/`POST_LIKE`/`COMMENT_LIKE`), postId, commentId, actorId, actorNickname, actorProfileImg, message, isRead | actorId·postId·commentId는 **FK 없이 단순 Long** 저장 (참조 대상 삭제 시에도 알림 유지). actorNickname·actorProfileImg는 **생성 시점 스냅샷** 복사 (join 불필요) |
| **Report** | reason(`SPAM`/`ABUSE`/`INAPPROPRIATE`/`FALSE_INFO`/`OTHER`) | FK: user_id, post_id(nullable), comment_id(nullable). `UK(user, post)`, `UK(user, comment)` |
| **RefreshToken** | userId, token, expiryDate | JWT Refresh Token 저장. Redis 전환 예정 |
