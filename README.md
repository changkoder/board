...
└────────────┘       │ content             │       │ nickname, profileImg     │
                     │ viewCount           │       │ role (USER/ADMIN)        │
                     │ likeCount           │       │ status (ACTIVE/BLOCKED)  │
                     │ commentCount        │       │ postCount, deleted       │
                     │ hidden, deleted     │       └──────────┬───────────────┘
                     └─┬─────────┬─────────┘                  │
                       │         │                            │1:N
                   1:N │     1:N │                            │
...
├─────────────┤├─────────────┤├──────────────┤├──────────┤├───────────┤
│ user_id FK  ││ user_id FK  ││ user_id FK   ││ user_id  ││ user_id   │
│ post_id FK  ││ comment_id  ││ post_id FK   ││ post_id  ││ post_id?  │
│ createdAt   ││ createdAt   ││ createdAt    ││ viewedAt ││ comment_id│
└─────────────┘└─────────────┘└──────────────┘└──────────┘│ reason    │
 UK(user,post)  UK(user,       UK(user,post)  UK(user,    │ createdAt │
                  comment)                      post)     └───────────┘
                                                          UK(user,post)
                                                          UK(user,comment)
...
├─────────────────────────────┤  ├────────────────┤
│ user_id (FK, 수신자)        │  │ userId         │
│ type (COMMENT / REPLY /     │  │ token          │
│   POST_LIKE / COMMENT_LIKE) │  │ expiryDate     │
│ postId, commentId (Long)    │  └────────────────┘
│ actorId (Long)              │
│ actorNickname  ┐            │  * Notification의 actorId, postId,
│ actorProfileImg┘ 스냅샷 저장│    commentId는 FK 없이 단순 Long
│ message, isRead, createdAt  │    → 참조 대상 삭제 시에도 알림 유지
└─────────────────────────────┘
```
