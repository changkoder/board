...
<details>
<summary><b>전체 엔티티 필드 상세</b></summary>

| 엔티티 | 주요 필드 | 비고 |
|---|---|---|
| **User** | id, email, password, nickname, profileImg, role(USER/ADMIN), status(ACTIVE/BLOCKED), postCount, deleted | BaseEntity 상속 (createdAt, updatedAt) |
| **Post** | id, title, content, viewCount, likeCount, commentCount, hidden, deleted | BaseEntity 상속. FK: user_id, category_id |
| **PostImage** | id, imageUrl, imageOrder | BaseEntity 상속. FK: post_id |
| **Comment** | id, content, likeCount, hidden, deleted | BaseEntity 상속. FK: user_id, post_id, parent_id(자기참조) |
| **Category** | id, name | BaseEntity 미상속 |
| **PostLike** | id, createdAt | FK: user_id, post_id. UK: (user_id, post_id) |
| **CommentLike** | id, createdAt | FK: user_id, comment_id. UK: (user_id, comment_id) |
| **Bookmark** | id, createdAt | FK: user_id, post_id. UK: (user_id, post_id) |
| **ViewLog** | id, viewedAt | FK: user_id, post_id. UK: (user_id, post_id) |
| **Notification** | id, type(COMMENT/REPLY/POST_LIKE/COMMENT_LIKE), postId, commentId, actorId, actorNickname, actorProfileImg, message, isRead, createdAt | FK: user_id(수신자). actorId 등은 단순 Long(FK 없음) |
| **Report** | id, reason(SPAM/ABUSE/INAPPROPRIATE/FALSE_INFO/OTHER), createdAt | FK: user_id, post_id(nullable), comment_id(nullable) |
| **RefreshToken** | id, userId, token, expiryDate | JWT Refresh Token 저장용 |
